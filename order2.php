    ...
    if (!isset($_POST['send'])) {
        ...
        <table border='1'>
            <form action="order2.php" method="post">
                ...
                <tr>
                    <th colspan="6">Введите ТФ</th>
                </tr>
                <tr>
                    <td colspan="6"><input type="text" name="phone"><br></td>
                </tr>

                <tr>
                    <th colspan="6">Введите Эмейл</th>
                </tr>
                <tr>
                    <td colspan="6"><input type="text" name="email"><br></td>
                </tr>

                <tr>
                    <th colspan="6">Введите адрес</th>
                </tr>
                <tr>
                    <td colspan="6"><textarea name="adress"></textarea><br></td>
                </tr>

                <tr>
                    <th colspan="6">Введите примечания к заказу</th>
                </tr>
                <tr>
                    <td colspan="6"><textarea name="prim" ></textarea><br></td>
                </tr>
...
<th>$totalsum</th>
                ...
                ?>
                <tr>
                    <td colspan='6' align="center"><input type="submit" name="send" value="Заказать"><br></td>
                </tr>
            </form>
        </table>
        <?php
    }else if (isset($_POST['send'])&&!empty($_POST['name'])&&!empty($_POST['phone'])&&!empty($_POST['email'])&&!empty($_POST['adress'])&&!empty($_POST['prim'])){
        ...
        $query="insert into klient(name,phone,email,adress,prim) VALUES('$name','$phone','$email','$adress','$prim')";

        require_once ("admin/param.php");

        mysqli_query($dbc,$query) or die("err zap q");
/////////////////////////////////////////////////////////////////////////////
        $idclient=mysqli_insert_id($dbc);//ф-я mysqli_insert_id возвращает айди только что добавленой записи в таблицу ВНИМАНИЕ ф-я работает только после mysqli_query которая в свою очередь содержит запрос инсерт инто добавление

        foreach ($_SESSION['items'] as $tmp){//в цикле перебираем корзину то есть копируем каждый элемент массива в переменную тмп, которую используем внутри цикла. но в данном случае в переменной тмп содержится массив полей одного товара

            $query2="insert into relation(idk,idt,kol_vo,d_z) VALUES ('$idclient','".$tmp['id']."','".$tmp['kol_vo']."',now())";//ф-я now() возвращает текущую дату сервера mysqli
/////////////////////////////////////////////////////////////////////////////
            mysqli_query($dbc,$query2) or die("err zap q2");
        }
        unset($_SESSION['items']);

        $_SESSION['items']=array();

        echo"<h5>Ваш заказ принят</h5>";
    }else{
        echo"Не достаточно данных для приема заказа";
    }
}
?>
